...
abstract class xPDOQuery extends xPDOCriteria {
    ...
    public function __construct(& $xpdo, $class, $criteria= null) {
        ...
        if ($class= $this->xpdo->loadClass($class)) {
            $this->_class= $class;
            $this->_alias= array_slice(explode('\\', $class), -1)[0];
                        ...
                        if (is_array($fkMeta['criteria']['local'])) {
                            foreach ($fkMeta['criteria']['local'] as $critKey => $critVal) {
                                if (is_numeric($critKey)) {
                                    ...
                                    $localCriteria["{$this->_alias}.{$critKey}"] = $critVal;
                                }
                            }
                        }
            ...
            if (isset ($conditions[0]) && !$this->isConditionalClause($conditions[0]) && is_array($pk) && count($conditions) == count($pk)) {
                ...
                while (list ($key, $val)= each($conditions)) {
                    if (is_int($key)) {
                        ...
                        $alias= $command == 'SELECT' ? $this->_alias : trim($this->xpdo->getTableName($this->_class, false), $this->xpdo->_escapeCharOpen . $this->xpdo->_escapeCharClose);
                        $operator= '=';
                        $conj = $conjunction;
